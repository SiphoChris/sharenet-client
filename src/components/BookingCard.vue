  ...
  <div class="border border-gray-300 rounded-lg p-6 shadow-md bg-white">
    ...
    >
      ...
      <button
        ...
        class="w-full mt-4 bg-primary-700 text-white py-2 rounded-md hover:bg-primary-600 hover:cursor-pointer transition"
      >
    ...
    <div v-else-if="booking.status === 'sold out'" class="mt-4 p-4 bg-red-100 border border-red-200 rounded-md">
      <p class="text-red-800 font-medium">This workshop is sold out.</p>
    </div>
  ...
  setup(props) {
    ...
    const confirmBooking = async () => {
      ...
      try {
        ...
        errorMsg.value =
          error.response?.data?.error || 'Failed to book this workshop. Please try again.';
      } finally {
